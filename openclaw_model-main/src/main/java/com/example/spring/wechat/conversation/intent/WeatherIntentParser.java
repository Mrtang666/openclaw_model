package com.example.spring.wechat.conversation.intent;


/**
 * 微信会话编排层组件，负责上下文、意图和回复组合。
 */
...
public class WeatherIntentParser {
    ...
    public boolean matches(String input) {
        ...
        if (WEATHER_KEYWORDS.stream().anyMatch(normalized::contains)) {
            return true;
        }
        return isFollowUp(normalized);
    }
    ...
    public boolean isFollowUp(String input) {
        ...
        return normalized.contains("呢")
                || normalized.contains("那")
                || normalized.contains("还")
                || normalized.contains("也")
