...
OpenClaw 是一个基于 Java 17 + Spring Boot 的多入口智能体项目，支持 CLI、微信 iLink、医疗照护 Web 控制台、小红书舆情管理台及多个外部 Sidecar。微信端 Agent 可处理文本、图片、语音、视频、文件，并通过 Function Calling 调用 53 个领域工具；当前能力还包括知识库/RAG、提醒、邮件收取与附件发送、医疗照护协同、小红书舆情采集分析和 HTML 长回复报告。

完整的当前功能、流程结构图、状态机、接口、定时任务、数据迁移和安全边界见：[项目功能与流程总览](docs/PROJECT_FEATURES_AND_FLOWS.md)。医疗照护专项说明见：[患者照护协同 Agent](docs/PATIENT_CARE_COORDINATION_AGENT.md) 和 [照护后端 API](docs/CARE_BACKEND_API.md)。
...
- 医疗身份入口：`/patient`、`/caregiver`、`/parents`、`/doctor`。
...
- 提醒：支持绝对时间、相对时间、每日/每周重复、列表、修改、取消、完成和延后，并由后台主动发送微信通知。
- 邮件：支持 QQ SMTP 纯文本邮件、非白名单确认、IMAP 查询邮件、读取正文和下载附件，以及确认后发送本地文件/代码目录附件。
- 医疗照护：支持患者、家属、医生身份，患者绑定与授权、可信记忆、每日签到、安全告警、照护计划、任务、状态摘要和审计。
- 微信长回复报告：长文本或多项结果可生成临时 HTML 报告链接，并按 TTL 自动清理。
...
原帖访问使用采集时获得的受限导航链接，并由管理台后端校验后跳转。升级到包含 `V30__add_xhs_post_access_urls.sql` 的版本后，旧帖子需要重新采集才能补齐访问链接；小红书仍可能要求浏览器登录、扫码或使用 App。Word 日报在请求时从数据库实时汇总并在内存中生成，不会默认保存到服务端目录。
