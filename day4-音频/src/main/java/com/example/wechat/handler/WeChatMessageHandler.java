...
public class WeChatMessageHandler {
    ...
    public String handleMessage(String userId, String message, byte[] imageData) {
        ...
        if (shouldUseVoice) {
            ...
            try {
                ...
                log.info("🎤 当前用户音色: {}, userId: {}", userVoice, userId);
    ...
    private boolean shouldExitVoiceMode(String message) {
        ...
        if (lower.contains("正常") && (lower.contains("回答") || lower.contains("回复"))) {
            return true;
        }
        return false;
    }
    ...
    private String handleVoiceSwitch(String userId, String message) {
        ...
        log.info("✅ 匹配到音色: {} -> {}", keyword, voice.name);
        memory.setUserVoice(userId, voice.name);
        log.info("✅ 音色已保存到 memory: {}", voice.name);
    ...
    private boolean isTtsTrigger(String message) {
        if (message == null) return false;
        String lower = message.toLowerCase();
        return lower.contains("语音回复") || lower.contains("语音形式") || lower.contains("用语音") || lower.contains("说给我听") || lower.contains("读给我听");
    }
    ...
    private String processImage(String userId, byte[] imageData, String userMessage) {
        try {
            ...
            Map<String, Object> params = Map.of("image_content", base64Image, "prompt", prompt);
